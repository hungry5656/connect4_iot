...
*/
    ...
    unsigned long ulBase = GSPI_BASE; //0x44021000; // set this when you have the mux_config file
    unsigned long recDummyData;

    SPICSEnable(ulBase); // enable  spi_CS
    GPIOPinWrite(GPIOA2_BASE, 0x2, 0x0); // set DC Pin (PIN_64) to low (low for command)
    GPIOPinWrite(GPIOA2_BASE, GPIO_PIN_6, 0x0); // set OLED_CS Pin (PIN_15) to low
    SPIDataPut(ulBase, c); // put the char to oled
    SPIDataGet(ulBase, &recDummyData); // collect dummy data
    GPIOPinWrite(GPIOA2_BASE, GPIO_PIN_6, GPIO_PIN_6); // set OLED_CS Pin (PIN_15) to high
    SPICSDisable(ulBase);
...
*/
    ...
    SPICSEnable(ulBase); // enable  spi_CS
    GPIOPinWrite(GPIOA2_BASE, 0x2, 0x2); // set DC Pin (PIN_64) to high (high for data)
    GPIOPinWrite(GPIOA2_BASE, GPIO_PIN_6, 0x0); // set OLED_CS Pin (PIN_15) to low
    SPIDataPut(ulBase, c); // put the char to oled
    SPIDataGet(ulBase, &recDummyData); // collect dummy data
    GPIOPinWrite(GPIOA2_BASE, GPIO_PIN_6, GPIO_PIN_6); // set OLED_CS Pin (PIN_15) to high
...
*/
  ...
  GPIOPinWrite(GPIOA3_BASE, GPIO_PIN_4, 0);	// RESET = RESET_LOW

  for(delay=0; delay<100; delay=delay+1);// delay minimum 100 ns

  GPIOPinWrite(GPIOA3_BASE, GPIO_PIN_4, 0x10);	// RESET = RESET_HIGH
