...
class connect4():
	...
	def gameOver(self, j, player):
		...
		for s in range(maxColumnIndex, minColumnIndex+1):
			...
			if count == 4:
				if self.visualize:
					pygame.draw.line(screen, WHITE, (int(j*SQUARESIZE+SQUARESIZE/2), int((s+2)
					                 * SQUARESIZE)), (int(j*SQUARESIZE+SQUARESIZE/2), int((s-2)*SQUARESIZE)), 5)
					pygame.display.update()
				return True
		# Left diagonal
		row = i
			...
			for r in range(self.shape[0]):
				if self.board[r][c] == 1:
					pygame.draw.circle(screen, RED, (int((c)*SQUARESIZE+SQUARESIZE/2),
					                   height-int((5-r)*SQUARESIZE+SQUARESIZE/2)), RADIUS)
